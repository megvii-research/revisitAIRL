...
class MoCov2Plus_Network(SiameseNetwork):
    def __init__(self, low_dim, hidden_dim, width, MLP, predictor, bn, CLS, queue_size, temperature):
        super(MoCov2Plus_Network, self).__init__(low_dim, hidden_dim, width, MLP, predictor, bn, CLS)
    ...
    def forward(self, inps):
        output_dict = super(MoCov2Plus_Network, self).forward(inps)
        pos1 = (self.norm(output_dict['online1']) * self.norm(output_dict['target2'])).sum(dim=-1, keepdim=True)
        pos2 = (self.norm(output_dict['online2']) * self.norm(output_dict['target1'])).sum(dim=-1, keepdim=True)
        neg1 = torch.mm(self.norm(output_dict['online1']), self.queue.clone().detach().t())
        neg2 = torch.mm(self.norm(output_dict['online2']), self.queue.clone().detach().t())
        logits1 = torch.cat((pos1, neg1), dim=1).div(self.temperature)
        logits2 = torch.cat((pos2, neg2), dim=1).div(self.temperature)
        labels = torch.zeros(logits1.size(0)).long().cuda()
        loss = self.criterion(logits1, labels) + self.criterion(logits2, labels)

        if np.random.rand() < 0.5:
            k_to_enqueue = output_dict['target1']
        else:
            k_to_enqueue = output_dict['target2']

        k_all = dist_collect(k_to_enqueue.detach())
        self.update_queue(self.norm(k_all))

        return {'loss': loss, 'logits': logits1, 'labels': labels, 'gp_feat': output_dict['gp_feat']}
...
class Trainer(BaseTrainer):
    ...
    def build_model(self):
        if 'model' not in self.__dict__:
            self.model = MoCov2Plus_Network(self.low_dim, self.hidden_dim, self.width, self.MLP,
                                             predictor=self.predictor, bn=self.bn, CLS=self.CLS,
                                             queue_size=self.queue_size, temperature=self.temperature)
        if 'classifier' not in self.__dict__ and self.CLS:
            self.classifier = nn.Linear(2048, 1000)

    def build_dataloader(self, args):
        if 'train_set' not in self.__dict__:
            from data.transforms import moco_transform, byol_transform
            from data.datasets import build_dataset

            if self.aug == 'byol':
                transform = byol_transform()
            elif self.aug == 'moco':
                transform = moco_transform(self.aug_plus)
            else:
                raise ValueError('Augmentation type {} is not supported.'.format(self.aug))

            self.train_set = build_dataset(transform, args.data_path, True)
    ...
    def build_optimizer(self, args):
        ...
        if self.CLS:
            self.optimizer_cls = torch.optim.SGD(self.classifier.parameters(), lr=self.lr_cls, momentum=0.9,
                                                 weight_decay=0.)
    ...
    def train(self, args, logger, writer):
        ...
        for i in range(self.ITERS_PER_EPOCH):
            ...
            if i % self.print_interval == 0 and args.rank == 0:
                writer.add_scalar('Train/Contrastive_Loss_Iter', to_python_float(reduced_con_loss),
                ...
                writer.add_scalar('Train/Inst_Top1_Iter', to_python_float(reduced_inst_top1),
                                  global_step=self.iter_count)
                writer.add_scalar('Train/Inst_Top5_Iter', to_python_float(reduced_inst_top5),
                                  global_step=self.iter_count)
        ...
        if args.rank == 0:
            logger.info(
                ...
                ' CLS-Loss: {:.2f}, Top-1: {:.2f}, Top-5: {:.2f}, Insta-Top-1: {:.2f},'
                ' Insta-Top-5: {:.2f}'.format(epoch + 1, args.total_epochs, lr, lr_cls,
